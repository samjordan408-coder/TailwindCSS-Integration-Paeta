const DATA_account = "./src/data/account_types.json";
...
const renderCards = (cards) => {
  ...
  cards.forEach((card) => {
    ...
    article.innerHTML = `
      <div class="w-12 h-12 mb-2.5 mx-auto flex items-center justify-center bg-sky-50 rounded-lg">
        <img src="${card.icon}" alt="${card.alt}" class="w-full h-full object-contain p-1.5">
      </div>

      <h3 class="text-[0.95rem] font-bold text-sky-500 mb-1.5 text-center">
        ${card.title}
      </h3>
